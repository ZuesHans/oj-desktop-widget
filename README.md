...
OJ 悬浮窗是一个 Windows 桌面悬浮 OJ 刷题统计工具，用来集中查看多个 OJ 账号的通过题数变化、补题记录和训练记录。
...
- 新增网页钩子同步 v1，可把每日总增量和题单公开字段同步到个人站点。
- 同步默认关闭，Token 使用系统安全存储，不写入普通配置 JSON。
- 默认同步不包含 OJ 用户名、账号级增量、密码、Cookie、令牌、题目备注或题解分析。
- 备注和题解分析需要单独开启后才会进入同步数据。
...
- 同步地址：例如 `https://example.com/api/oj-sync`
- 同步密钥：服务端配置的 Bearer Token

同步数据只用于公开投影展示。默认字段包括：
...
- 默认所有数据只保存在本机应用目录。
- 应用不会上传密码、Cookie 或 OJ 令牌。
- 备份文件不包含同步密钥。
- 网页钩子同步为可选功能，默认关闭。
- 网页钩子同步默认不发送 OJ 用户名和账号级增量。

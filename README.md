...
OJ Float 使用 JSON 备份，而不是直接复制数据库文件。轮换自动备份带有独立 schema 版本和 SHA-256 内容校验；手动便携备份也带有格式版本，并在写入后立即重读解析。当前应用数据仍由多个本地 JSON 存储，使用独立、可读且严格校验的备份文件，可以避免把临时文件、抓取缓存或安全存储中的 Token 一并复制，也便于未来升级存储实现时继续恢复。
